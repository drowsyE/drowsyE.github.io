...
+) 32KB ~ 64KB가 일반적이며, NVIDIA H100은 227KB를 제공한다.(80GB 한대가 4천만원씩이나 한다..)

또한 그래픽스 API를 사용하는 경우, 어떤 API를 사용하냐에 따라서도 공유 메모리를 효율적으로 할당받기 어렵다. DX11의 경우는 쓰레드 그룹당 사용할 수 있는 공유 메모리의 최대 크기는 32KB로 고정되어 있다.
...
5. 구한 block sum들의 prefix sum을 각 블록의 local scan 결과에 더해준다.
